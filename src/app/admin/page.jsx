...
export default function AdminPage() {
  ...
  useEffect(() => {
    ...
    console.log(cancelledOrdersList.length);
          ...
          {active === 'orders' && (
            <motion.div key="orders" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
              <FilterBar
                ...
                filters={[{
                  key: 'status', value: orderStatus, onChange: setOrderStatus, options: [
                    { value: 'all', label: 'All statuses' },
                    ...['pending', 'processing', 'shipped', 'delivered', 'cancelled', 'returned'].map(s => ({ value: s, label: s.charAt(0).toUpperCase() + s.slice(1) }))
                  ]
                }]}
              />
