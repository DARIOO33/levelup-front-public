  ...
  useEffect(() => {
    ...
    reviewsApi.getForProduct(id)
      .then(r => {
        ...
        const avg = r.data.avg || null;
        setReviews(revs);
        setReviewCount(count);
        setReviewAvg(avg);
        // Inject aggregateRating into the server-rendered JSON-LD Product schema
        // so Google can show star ratings in search results and image search
        if (count > 0 && avg) {
          try {
